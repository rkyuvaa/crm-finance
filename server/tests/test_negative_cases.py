...
def test_negative_cases_and_validations(seeded_client, db):
    ...
    not_found_proj = seeded_client.get("/api/v1/projects/999999", headers=admin_headers)
    assert not_found_proj.status_code == 404
    assert "Project not found" in not_found_proj.json()["detail"]

    not_found_task = seeded_client.get("/api/v1/tasks/999999", headers=admin_headers)
    ...
    p_resp = seeded_client.post("/api/v1/projects", json={"name": "Negative Test Project"}, headers=admin_headers)
    assert p_resp.status_code == 201
    p_id = p_resp.json()["id"]

    t_resp = seeded_client.post("/api/v1/tasks", json={"title": "Negative Test Task", "project_id": p_id}, headers=admin_headers)
    assert t_resp.status_code == 201
    t_id = t_resp.json()["id"]

    invalid_subtask = seeded_client.post(f"/api/v1/tasks/{t_id}/subtasks", json={}, headers=admin_headers)
    assert invalid_subtask.status_code == 422

    invalid_comment = seeded_client.post(f"/api/v1/tasks/{t_id}/comments", json={}, headers=admin_headers)
    assert invalid_comment.status_code == 422

    # 5. Invalid Foreign Key reference
    invalid_fk_subtask = seeded_client.post("/api/v1/tasks/999999/subtasks", json={"title": "Orphan Subtask"}, headers=admin_headers)
    assert invalid_fk_subtask.status_code == 404

    invalid_fk_comment = seeded_client.post("/api/v1/tasks/999999/comments", json={"content": "Orphan Comment"}, headers=admin_headers)
    assert invalid_fk_comment.status_code == 404

    invalid_fk_attachment = seeded_client.post("/api/v1/tasks/999999/attachments", json={"filename": "orphan.png"}, headers=admin_headers)
