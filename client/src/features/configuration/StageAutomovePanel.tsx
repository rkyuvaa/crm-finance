...
const formSchema = z.object({
  ...
  target_status: z.string().min(1, 'Target stage is required'),
...
function AutomoveRuleDialog({
  ...
  const {
    ...
    defaultValues: {
      ...
      field_name: editing?.field_name ?? 'vehicle_model_id',
      ...
      target_status: editing?.target_status ?? 'VERIFICATION',
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle sx={{ fontWeight: 700 }}>
        {editing ? 'Edit Stage Auto-Move Rule' : 'Create Stage Auto-Move Rule'}
      </DialogTitle>
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogContent>
          <TextField
            ...
            label="Rule Name *"
            margin="dense"
            placeholder="e.g. Auto-move to Verification when Finance Company selected"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
            <div>
              ...
              >
                <MenuItem value="standard_field">Standard Lead Field</MenuItem>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
            ...
            <div>
              ...
              <Select
                fullWidth
                size="small"
                value={watch('target_status') || 'VERIFICATION'}
                onChange={(e) => setValue('target_status', e.target.value)}
              >
                {stages.map((s) => (
                  <MenuItem key={s.key} value={s.status || s.key.toUpperCase()}>
                    {s.label} ({s.status || s.key.toUpperCase()})
                  </MenuItem>
                ))}
              </Select>
            </div>
...
export default function StageAutomovePanel() {
  ...
  const { data: rules = [], isFetching, isError, refetch } = useAutomoveRulesQuery();
      ...
      <Paper sx={{ border: '1px solid #E4EBE1', borderRadius: '14px', overflow: 'hidden' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: '#16231B' }}>
                Field-Based Stage Auto-Move Rules
              </span>
            </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          {isFetching && !rules ? (
            ...
            <EmptyState
              title="No Auto-Move Rules Configured"
              hint={isAdmin ? 'Click "Add Auto-Move Rule" to create automated stage workflow triggers.' : 'Contact an admin to set up stage auto-move rules.'}
            />
          ) : (
                ...
                {rules.map((r) => (
                  <tr key={r.id} style={{ opacity: r.is_enabled ? 1 : 0.55 }}>
                    ...
                    <td style={{ padding: '11px 16px', borderBottom: '1px solid #F0F4EE', color: '#44584C', fontSize: 12 }}>
                      {r.condition_operator === 'is_filled' && 'Is Filled / Provided'}
                    ...
                    <td style={{ padding: '11px 16px', borderBottom: '1px solid #F0F4EE' }}>
                      <Chip label={r.target_status} size="small" color="success" sx={{ fontWeight: 700, fontSize: 11 }} />
                    </td>
                    <td style={{ padding: '11px 16px', borderBottom: '1px solid #F0F4EE' }}>
                      <Switch
                        size="small"
